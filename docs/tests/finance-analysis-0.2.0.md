...
## 用例 9b：基金对比命题（ETF vs 主动）

**命题**：我想配置美股半导体，买 SMH 还是某主动半导体基金？

**预期路由**：
- 加载 `fund-analysis`（主）+ `top-down-selection`（赛道锚）+ `ai-thematic-investing`（半导体在 AI 链位置）

**预期输出字段**：
- [ ] 两只基金分类识别（SMH = 被动 ETF / 主动 = 主动股票型）
- [ ] 核心指标对比（跟踪误差 / α / 夏普 / 最大回撤 / 费率）
- [ ] 主动基金 Step 2 管理人扫描（任职 / 规模 / 历史）
- [ ] 持仓分析（风格稳定性 / 集中度）
- [ ] 决策卡（推荐哪只 + 仓位 + 退出信号）
- [ ] **最高原则**：是否提及"看 α 不看绝对收益"

**失败判定**：
- 只看年化收益不看夏普 → 违反最高原则
- 主动基金 α < 2% 还推荐主动 → 覆盖不了费率差
- 不看管理规模（> 500 亿还推主动）→ 规模陷阱
- 不看流动性（ETF 日均成交 < $5M 还推荐）→ 流动性陷阱

---

## 用例 9c：债券配置命题（0.3.0）

**命题**：Fed 降息周期下，TLT 能买吗？

**预期路由**：
- 加载 `bond-analysis`（主）+ `top-down-selection`（§4.1.2 加息降息预期锚）

**预期输出字段**：
- [ ] 债券分类（利率债 / 超长久期 ETF）
- [ ] 久期识别（TLT ~18）+ 利率敏感度（1% 利率变化 ≈ 18% 价格变化）
- [ ] 宏观锚：当前 Fed 利率路径 + 降息预期（CME FedWatch 数据）
- [ ] 四维评分（久期匹配 / 信用质量 / YTM 吸引力 / 流动性）
- [ ] **最高原则**：是否说明"债券不是保本，是利率+信用的 bet"
- [ ] 配置权重 + 止损 + 退出信号

**失败判定**：
- 说"国债保本" → 违反最高原则
- 不看久期直接看 YTM → 漏了最关键维度
- 不调 top-down-selection 加息降息预期 → 脱离宏观锚
- 忽视 2022 年 TLT -45% 历史教训
