...
第一次会话有两个任务：快速建立框架文件，然后**立即开始第一个功能的开发**。
...
**实现**：每完成一个有意义的步骤就 git commit，便于回溯与延续。
...
**绝不留下的状态**：编译不通过、运行报错、数据库迁移没跑。下一个会话的第一件事是 `bash init.sh`，这一步必须能成功。
...
初始化会话选择的第一个功能**跳过 Plan**，直接编码——初始化阶段已有全面了解，通常是基础功能。从第二个功能开始按复杂度判断是否走 Plan。
